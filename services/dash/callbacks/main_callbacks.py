...
from utils.style import apply_dark_theme  # ⬅️ додали
...
def _log_filters(dem, lulc, landform, slope, hand_range_) -> None:
    logger.info(
        ...
        dem, lulc if lulc else "-", landform if landform else "-", slope if slope else "-", hand_range_ if hand_range_ is not None else "-",
    )
...
@callback(
    ...
    Input("idx-tabs", "value"),          # ⬅️ узгодили id
...
# ---------- головний контролер усіх вкладок ----------
@callback(
    Output("idx-tab-content", "children"),  # ⬅️ віддаємо готовий контент вкладки
...
def update_dashboard(
    ...
    # ── TAB 1: перший показ без натискання "Оновити"
    if tab == "tab-1" and (n_clicks is None or n_clicks == 0):
        try:
            ...
            for f in (hist_fig, box_fig, filtered_bar):   # ⬅️ темна тема
                apply_dark_theme(f)
    ...
    if tab == "tab-1":
        ...
        try:
            ...
            logger.info("render tab-1 filtered | dem=%s stats=%d sample_n=%d",
                        dem, len(filtered_stats_all_dems), len(sample_df or []))
