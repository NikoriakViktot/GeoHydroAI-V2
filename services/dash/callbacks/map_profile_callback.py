...
def get_geojson_for_date(track, rgt, spot, dem, date, hand_range=None, step=50):
    """
    Читаємо потрібні колонки з parquet через DuckDB і повертаємо FeatureCollection.
    delta = COALESCE(delta_{dem}, orthometric_height - h_{dem})
    """
    h_dem_col = f"h_{dem}"
    hand_col  = f"{dem}_2000"
    delta_expr = f"COALESCE(delta_{dem}, orthometric_height - {h_dem_col})"

    hand_sql = ""
    if hand_range and len(hand_range) == 2 and all(v is not None for v in hand_range):
        lo, hi = float(hand_range[0]), float(hand_range[1])
        hand_sql = f" AND {hand_col} IS NOT NULL AND {hand_col} BETWEEN {lo} AND {hi}"

    sample_sql = ""
    if isinstance(step, int) and step > 1:
        sample_sql = f"WHERE rn % {int(step)} = 1"

    sql = f"""
        WITH base AS (
            SELECT x, y, orthometric_height, {h_dem_col} AS h_dem, {delta_expr} AS delta
            FROM {T_TRACKS}
            WHERE track = {float(track)} AND rgt = {float(rgt)} AND spot = {float(spot)}
              AND DATE(time) = '{date}'
              AND atl03_cnf = 4 AND atl08_class = 1
              AND orthometric_height IS NOT NULL AND {h_dem_col} IS NOT NULL
              {hand_sql}
        ),
        numbered AS (
    ...
    for _, row in df.iterrows():
        feats.append({
            ...
            "properties": {
                ...
                "orthometric_height": float(row["orthometric_height"]),
            },
        })
...
def update_map_markers(selected_profile, hand_range, hand_toggle):
    """Малюємо лише маркери на мапі."""
    if not selected_profile:
        return []

    track_str = selected_profile.get("track")
    dem = selected_profile.get("dem")
    date = selected_profile.get("date")
    if not (track_str and dem and date):
        return []
    ...
    hand_range_for_query = (
        hand_range if (use_hand and hand_range and len(hand_range) == 2
    ...
    valid = [(i, f) for i, f in enumerate(feats)
             if f["properties"].get("delta") is not None]
    if not valid:
        return []

    deltas = [f["properties"]["delta"] for _, f in valid]
    min_i = valid[deltas.index(min(deltas))][0]
    max_i = valid[deltas.index(max(deltas))][0]

    pts = []
    for i, f in enumerate(feats):
        lon, lat = f["geometry"]["coordinates"]
        d = f["properties"]["delta"]
        if i == min_i:
            pts.append(dl.CircleMarker(
                center=[lat, lon], radius=5, color="lime", fillColor="lime",
                fillOpacity=1, children=[dl.Tooltip(f"Min: {d:.2f} м")]))
        elif i == max_i:
            pts.append(dl.CircleMarker(
                center=[lat, lon], radius=5, color="red", fillColor="red",
                fillOpacity=1, children=[dl.Tooltip(f"Max: {d:.2f} м")]))
        else:
            pts.append(dl.CircleMarker(
                center=[lat, lon], radius=2, color="blue", fillColor="blue",
                fillOpacity=0.8, children=[dl.Tooltip(f"{d:.2f} м")]))
    return pts
...
# def get_geojson_for_date(track, rgt, spot, dem, date, hand_range=None, step=50):
#     """
#     GeoJSON з DuckDB: читаємо лише потрібні колонки з parquet через T_TRACKS,
#     рахуємо/беремо delta, фільтруємо та робимо підсемпл на рівні SQL.
#     """
#     h_dem_col = f"h_{dem}"
#     hand_col  = f"{dem}_2000"
#     # якщо delta_{dem} є в даних — візьмемо її, інакше порахуємо як (orthometric_height - h_dem)
#     delta_expr = f"COALESCE(delta_{dem}, orthometric_height - {h_dem_col})"
#
#     hand_sql = ""
#     if hand_range and len(hand_range) == 2 and all(v is not None for v in hand_range):
...
#         hand_sql = f" AND {hand_col} IS NOT NULL AND {hand_col} BETWEEN {lo} AND {hi}"
#
#     # підсемпл робимо в SQL через ROW_NUMBER(), щоб менше тягнути в Python
#     sample_sql = ""
#     if isinstance(step, int) and step > 1:
#         sample_sql = f"WHERE rn % {int(step)} = 1"
#
...
#               AND {h_dem_col} IS NOT NULL
#               {hand_sql}
#         ),
#         numbered AS (
#             SELECT
#                 x, y, orthometric_height, delta,
#                 ROW_NUMBER() OVER (ORDER BY x) AS rn
#             FROM base
#             WHERE delta IS NOT NULL
#         )
...
#
#
# # --- (a) Дропдауни (залишаємо як є, тільки без хардкоду шляху)
# @callback(
