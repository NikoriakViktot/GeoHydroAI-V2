...
def compute_dem_difference(path1, path2):
    """diff = DEM2 - DEM1, DEM2 автоматично ресемплиться/репроєктується в грід DEM1."""
    dem1 = DEM(path1)  # має data, transform, profile (з crs, nodata)
...
def plot_histogram(diff_array, bins=60, clip_range=(-50, 50)):
    ...
    if valid.size == 0:
        ax.set_title("No data after clipping"); ax.axis("off")
    else:
        ax.hist(valid, bins=bins, color="skyblue", edgecolor="black")
        ax.set_title("Histogram of Elevation Errors"); ax.set_xlabel("Error (m)"); ax.set_ylabel("Frequency")
...
def calculate_error_statistics(diff_array: np.ndarray):
    valid = diff_array[~np.isnan(diff_array)]
    if valid.size == 0:
        return {"count":0,"mean_error":np.nan,"median_error":np.nan,"std_dev":np.nan,
                "rmse":np.nan,"min":np.nan,"max":np.nan,"skewness":np.nan,"kurtosis":np.nan}
    return {
